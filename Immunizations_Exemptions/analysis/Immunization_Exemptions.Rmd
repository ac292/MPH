---
title: "Vaccination Exemptions for Kindergartners in the United States"
...
According to reports in the media, there is a growing proportion of parents of young children, who hesitate or refuse to vaccinate their children in the United States. Several immunizations are state-mandated for children in public schools. Amongst those are Tetanos, Diphteria, Pertussis, Hepatitis B, Measles, Mumps, Rubella, Varicella. Although all states allow children to be exempted from immunization for medical reasons, some states are more stringent than others in allowing exemptions to immunization for philosophical or religious reasons. Under the leadership of the new secretary of health and human services, R. F. Kennedy Jr., federal guidelines for recommended childhood immunizations are shifting and there will be consequences in the stringency of immunization mandates for children attending public schools at the level of states. 

## Project Question

Data is available at the CDC that reports on the level of immunization and exemptions granted for medical and non-medical reasons for children enrolled in public Kindergarten classes for the years spanning 2019 to 2024. We will focus first on analyzing the level of non-medical exemptions granted by the various States and follow the evolution of this trend over the years where data is publicly available. We will also look at whether the Covid epidemic had a visible impact on the level of non-medical exemptions granted. Later, we will also inquire about how shifting attitudes towards vaccinations, reflected by shift in states mandates, impact the levels of non-medical exemptions granted. Thirdly, we will analyze outbreaks of diseases that are covered by childhood immunizations and attempt to see if they correlate, as expected, with varied attitudes towards immunizations in the different geographical regions. 

## Data Set Overview

We will use the *Vaccination Coverage among Kindergartners* data set available at [https://data.cdc.gov/Vaccinations/Vaccination-Coverage-and-Exemptions-among-Kinderga/ijqb-a7ye/about_data]. The population studied is that of children enrolled in Kindergarten in public schools in the United states. The study reports on vaccination and exemption rates for the school years spanning 2009-2010 to 2024-2025. The data was collected mainly through the census report, which is a mandated collection of data reported by all public school administrators. 
...
*We count the number of observations in the original data set and also the number of geographical areas surveyed and the type of survey that fed the data set.*
...
*The data set contains data that is either redundant or un unnecessary for the analysis planned at this time. We create a new data frame after removing this data that we do not deem necessary at this time. We will go back to the original data set for following analysis. 
...
\begin{itemize} \color{blue} \item `r unique(cleaned_data_K$geography) ` \end{itemize}

We will create a new data set that only contains data per state- we will remove the data for the whole USA and the data that is specific to cities (NYC, Houston, San Antonio).
...
The data set draws from a very large population: for example: the number of children for which Varicella vaccination data was reported in school year 2024-25 is `r pop_K`. The average number of children surveyed per geographical area is `r mean_pop_K`. Let's look at a histogram of population surveyed for Varicella immunization in school year 2024-25.
...
*What is the time period covered by the study? Indicate all school years sampled* \
...
* \color{blue} The study spans from `r min(cleaned_data_K$year)` to `r max(cleaned_data_K$year)`.  We need to create a new numerical column "year" to describe the 20XX-XX+1 school year as 20XX. \color{black}
...
* \color{blue} The estimate value of vaccination coverage is a percentage and thus should be a numerical value. It is encoded in the original data set as a character. It needs to be re-encoded as a numerical value. First step is to remove all the NR or NA values in the estimate column. \color{black} 
```{r}
...
```
* \color{blue} The estimate value of vaccination coverage is a percentage and is now ready to be encoded in a numerical value.  \color{black}
...
# Check output for first line of data after conversion and compare with same before conversion
...
```
* \color{blue} Vaccination coverage is now recorded as a numerical value.\ The face value of the first line of data was `r data1a` before class conversion and is now still `r data1b`, hence the data was not altered during class conversion. \color{black}

# Focus on Vaccination Exemptions
...
## Raw plotting of Exemptions per State
...
* \color{blue} The geographical area that has the highest level of vaccination exemptions  (`r max(data_exemption$estimate)`%) is  `r year_max_exemption$geography` in the school year starting `r year_max_exemption$year `. The state that has the lowest level of Non-Medical Exemptions is Maine. \color{black}
...
*For comparison, let's look at the data from Maine, which has the lowest % of Non-Medical Exemptions*
...
- The general trend has been an increase in the number of non-medical exemptions from 2009 to 2025. Several states show non-medical exemptions rates reaching 10% or more for the 2024-25 school year, notably Alaska, Arizona, Idaho, North Dakota, Utah, Oregon. 

## Idaho


- The Idaho plot shows that the vaccination exemption rate in Idaho has steadily increased from a low of `r min(data_exemption_Idaho_nonmed$estimate)`\% since data was first collected in `r min(data_exemption_Idaho_nonmed$year)`, to reach its highest level `r max(data_exemption_Idaho_nonmed$estimate)`\% in `r max(data_exemption_Idaho_nonmed$year)`, which is the last year for which data is available. 
...
- The Maine plot shows that the vaccination exemption rate in the state of Maine has decreased from a high of
`r max(data_exemption_Maine_nonmed$estimate)`\% reached in `r year_max_exemption_Maine_nonmed$year`, to reach its lowest level `r min(data_exemption_Maine_nonmed$estimate)`\% in `r year_min_exemption_Maine_nonmed$year`. 
